"""
  ...
  with variable wave-speed: c = c(x)

  u_xx = 1/c^2 u_tt + F(x,t) , domain = (0, 2 pi)
  u(x,t) = sin(x) cos(t), analytical solution
  u(0,t) = u(2 pi,t) = 0  boundary conditions
  u(x,0) = sin(x), initial condition
  u_t(x,0) = -sin(x) sin(t) = 0
  F(x,t) = sin(x) cos(t) (1/c^2 - 1)
  c^2 (x) = 1 + 1/2 * sin(x)
  
  observation: when I expand the time-window 
  from (0, pi) to (0, 2 pi), I get poor results.
"""
...
class PDE_PINN():
    ...
    def __init__(self, 
                 ...
                 domain = (0, 1, 0, 1), 
                 n_collocation_pts = 2000, # this will appear as batch size                
                 n_initcond_pts = 128,
                 n_boundary_pts = 128):
    ...
    #@tf.function
    # if I make this a tf.function, I cannot do numpy operations.
    def construct_wave_speed(self, x):
        # wave_speed(x)^2 profile:
        # 1. allocate:
        self.c2 = np.zeros(x.size)
        # 2. construct:
        for i in range(x.size):
            self.c2[i] = 1.0 + 0.5 * np.sin(x[i])
        # 3. convert to tensor:
        self.c2_tensor = tf.convert_to_tensor(self.c2, dtype=tf.float32)
        return

    #@tf.function
    # c2[i] needs to be constructed first by calling construct_wave_speed
    def construct_source_term(self, x, t):
        # 1. allocate:
        self.F = np.zeros(x.size)
        # 2. construct:
        for i in range(x.size):
            self.F[i] = np.sin(x[i]) * np.cos(t[i]) * (1.0/self.c2[i] - 1.0)
        # 3. convert to tensor (did not improve performance):
        self.F_tensor = tf.convert_to_tensor(self.F, dtype=tf.float32)    
        return
    ...
    def pde_residual(self, x, t):
        """
        PDE residual:= u_xx - 1/c^2 * u_tt - F(x,t)
        """
        u    = self.compute_u(x, t)
        ...
        f = u_xx - 1.0/self.c2_tensor * u_tt - self.F_tensor
        return f
    ...
    def collocation_points(self):
        ...
        # construct wave_speed profile:
        self.construct_wave_speed(x)
        
        # construct source term in the PDE:
        self.construct_source_term(x, t)
...
# ----------------------------------------
# 1. problem setup
domain = (0, 2 * math.pi, 0, 1 * math.pi)
...
for x_i in range(grid_density):
    for t_i in range(grid_density):
        x = xg[x_i][t_i]
        t = tg[x_i][t_i]
        u_exact[x_i][t_i] = np.sin(x) * np.cos(t)
